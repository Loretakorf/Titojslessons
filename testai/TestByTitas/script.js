...
const prefillForm = (order) => {
  ...
  premiumPackageCheckbox.checked = parseStringifiedBoolean(
    order.premiumPackage
  );
...
const person1 = createPerson("L", "p", 5);
...
const validateIfPersonCanPurchase  = () => {
  const loans = 5000;
  if(loans > 10000) {
    return false;
  }
  if(housePrice > budzet) {
    return false;
  }
  ...
  console.log({loans, housePrice, budzet, bankAccountId});
  informBank();
  return true;
}
console.log((validateIfPersonCanPurchase(housePrice)));
